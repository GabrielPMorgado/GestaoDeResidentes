    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white mb-1">Profissionais Inativos</h1>
            <p className="text-slate-400 text-sm">Gerencie profissionais que foram inativados no sistema</p>
          </div>
        ...
        <div className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-slate-700/50 overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-slate-300 font-medium mb-1">Nenhum profissional inativo</p>
              <p className="text-slate-500 text-sm">Não há profissionais inativos no momento</p>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-slate-900/50 border-b border-slate-700/50">
                    <tr>
                      <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Profissional</th>
                      <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">CPF</th>
                      <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Especialidade</th>
                      <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Registro</th>
                      <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Telefone</th>
                  ...
                  <tbody className="divide-y divide-slate-700/50">
                    {paginacao.dados.map((profissional) => (
                      <tr key={profissional.id} className="hover:bg-slate-700/20 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="text-sm font-semibold text-white">{profissional.nome_completo}</div>
                              {profissional.email && (
                                <div className="text-xs text-slate-400 mt-0.5">{profissional.email}</div>
                              )}
                            </div>
                          </div>
                        </td>
              ...
              {paginacao.paginas > 1 && (
                <div className="flex items-center justify-between px-6 py-4 bg-slate-900/30 border-t border-slate-700/50">
                  <div className="text-sm text-slate-400">
                    Mostrando <span className="font-semibold text-slate-300">{paginacao.dados.length}</span> de <span className="font-semibold text-slate-300">{paginacao.total}</span> profissionais
                  </div>
